...
export default function OwnerAdminPanelDetails() {
  const { isOpen, openModal, closeModal } = useModal();

  const [companyDetails, setCompanyDetails] = useState({
    name: "TapDay Pvt Ltd",
    owner: "Umar Azeem",
    email: "info@tapday.com",
    regId: "REG-2025-TAP",
  });

  const handleDelete = (field: keyof typeof companyDetails) => {
    setCompanyDetails((prev) => ({ ...prev, [field]: "" }));
  };

  const [editValues, setEditValues] = useState(companyDetails);

  const handleSave = () => {
    setCompanyDetails(editValues);
    closeModal();
  };

  return (
    <>
      <div className="p-5 border border-gray-200 rounded-2xl dark:border-gray-800 lg:p-6 overflow-scroll">
                ...
                <div key={key} className="space-y-1">
                  ...
                  <div className="flex justify-between items-center gap-2">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        onClick={() =>
                          handleDelete(key as keyof typeof companyDetails)
                        }
          ...
          <form className="flex flex-col">
            <div className="px-2 overflow-y-auto custom-scrollbar">
              <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                ...
                <div>
                  ...
                  <Input
                    type="text"
                    value={editValues.regId}
                    onChange={(e) =>
                      setEditValues({ ...editValues, regId: e.target.value })
                    }
                  />
                </div>
              </div>
            ...
            <div className="flex items-center gap-3 px-2 mt-6 lg:justify-end">
              ...
              <Button
                size="sm"
                onClick={handleSave}
            className="bg-[#FF9900]  text-white hover:bg-[#e58c00]"
              >
                Save Changes
              </Button>
            </div>
